...
void noncov_mp2_fused(Tensor<double> Int, Tensor<double> C, Tensor<double> Phat,
  ...
  TensorVar IntCPhat =
      TensorVar("IntCPhat", Type(Int.getComponentType(), {PAO_HAT}), {taco::dense});
...
void nofilter_nary(Tensor<double> Int, Tensor<double> C, Tensor<double> X,
  ...
  std::cout << "Time nofilter_nary:  " << elapsed.count() << " ms "
            << std::endl;
  // write("data_3cent/X_nofilter_nary.tns", X);
}

int main() {
  //Tensor<double> Int("Int", {PAO, PAO, AUX}, {Dense, Sparse, Sparse});
  Tensor<double> Int = read("data_3cent_realmid/Int.tns", {Dense, Sparse, Sparse});
  Int.pack();
  Int = Int.transpose({2, 0, 1});
  Int.setName("Int");

  //Tensor<double> L("L", {AUX, MO}, {Dense, Sparse});
  Tensor<double> L = read("data_3cent_realmid/L.tns", {Dense, Sparse});
  L.pack();
  L.setName("L");

  Tensor<double> Phat("Phat", {PAO, PAO_HAT}, {Dense, Sparse});
  Phat = read("data_3cent_realmid/Phat.tns", {Dense, Sparse});
  Phat.pack();
  Phat.setName("Phat");

  Tensor<double> C("C", {PAO, MO}, {Dense, Sparse});
  C = read("data_3cent_realmid/C.tns", {Dense, Sparse});
  Phat.pack();
  C = C.transpose({1, 0});
  C.setName("C");

  //Tensor<double> X("X", {AUX, MO, PAO_HAT}, {Dense, Sparse, Sparse});
  //noncov_mp2_fused(Int, C, Phat, X, L);
  ...
  Tensor<double> X_nofilter("X", {AUX, MO, PAO_HAT}, {Dense, Sparse, Sparse});
